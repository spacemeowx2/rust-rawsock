...
pub extern "C" fn on_received_packet(user: * mut c_uchar, h: * const PCapPacketHeader, bytes: * const c_uchar) {
    let callback: &mut Box<&mut FnMut(&BorrowedPacket)> = unsafe{transmute(user)};
